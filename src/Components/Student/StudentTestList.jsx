...
function StudentTestList({clicked, setClicked}) {
  ...
  const Join = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3001/StudentTestList?uid=${uidSection.toUpperCase()}`
      );
      setUidSection("");
      response.data.map((test) => {
        setSectionName(test.Section_Name);
        setSubject(test.Subject);
      });
      if (response.status === 200) {
        const response1 = await axios.get(
          `http://localhost:3001/CheckingTestList?Subject=${subject}&Section_Name=${sectionName}`
        );
        response1.data.map((test) => {
          setTestName(test.TestName);
          setUidTest(test.Uid_Test);
        })
        if (response1.status === 200) {
          fetchingStudentTest();
          const response2 = await axios.put(
            `http://localhost:3001/StudentTestList?StudentUid=${TUPCID}&TestName=${testName}&UidTest=${uidTest}`,
            response.data
          );
  ...
  const fetchingStudentTest = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3001/FetchingStudentTest?StudentUid=${TUPCID}`
      );
      if (response.status === 200) {
  ...
  return (
    <main className="w-100 min-vh-100 d-flex justify-content-center">
      <section className="contatiner col-12 text-sm-start text-center d-flex flex-column align-items-start p-2">
        ...
        <i className="d-block d-sm-none bi bi-list fs-5 pe-auto bg-danger px-2 rounded" onClick={handleclick}></i>
          <h2 className="m-0">STUDENT</h2>
        </div>
