  ...
  const login = async () => {
    ...
    try {
      ...
      if(response.status === 200){
        router.push(`/Admin_Page/Admin?Username=${response.data.Username}`)
            ...
            />
             {showPassword ? (
                ...
                onClick={() => setShowpassword(!showPassword)}
                  className="bi bi-eye-slash text-dark fs-4 position-absolute custom-eye"
                  style={{ width: "36px" }}
                ></i>
              ) : (
                <i
                  onClick={() => setShowpassword(!showPassword)}
                  className="bi bi-eye text-dark fs-4 position-absolute custom-eye"
                  style={{ width: "36px"}}
                ></i>
              )}
          </div>
