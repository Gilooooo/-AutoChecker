...
app.get("/CheckTestName", async (req, res) => {
  ...
  try {
    const query =
      "SELECT COUNT(*) as count FROM faculty_testlist WHERE TestName = ?";
...
app.get("/generateTestPaperdoc/:uid", async (req, res) => {
  try {
    ...
    addStyledParagraph("2. PUT YOUR ANSWER INSIDE THE CORRESPONDING BOXES", {
      bold: true,
    });
...
app.post("/sendanswertoresult", async (req, res) => {
  const { TUPCID, UID, results, correct, wrong, totalscore2, maxscore } =
    req.body;
...
app.get("/myresult/:TUPCID/:uid", async (req, res) => {
  ...
  try {
    ...
    if (studentScores.length > 0) {
      ...
      console.log("studentlist:", resultlist);
...
app.put("/updateTotalScore/:TUPCID", async (req, res) => {
  try {
    ...
    const updateScoreQuery =
      "UPDATE student_results SET CORRECT = ?, WRONG = ?, TOTALSCORE = ? WHERE TUPCID = ?";
